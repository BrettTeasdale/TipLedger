...
public class LineChartMonthlyActivity extends BaseActivity {
    /**
    ...
    /**
     * Initialize the LineChartV2MonthlyActivity activity
     *
            ...
            for (JobEntity jobEntity : mJobEntities) {
                ...
                for (LedgerEntryEntity ledgerEntryEntity : tempLedger) {
                    ...
                    if (nextItemEpochMilli >= nextEpochMilli || endOfCurrentLedger) { // add values to dataset
                        ...
                        if(nextItemEpochMilli >= nextNextEpochMilli || endOfCurrentLedger) { // add zero value if day after has no values
